...
# TESTE
    # Add the 0,0,0.5 point to the p3d -> 4 red dots
    # Add the 0,0,0 point to the output_points
    points_mask = np.hstack([p3d, [[0], [0], [0.05]]])
    black_mask = np.zeros((3, p3d.size//3)).astype(np.float64)
    colors_mask = np.hstack([black_mask, [[255], [0], [0]]])
    p3d = np.hstack([p3d, np.array([[0.0], [0.0], [0.5]])])
    color = np.vstack([color, [255, 0, 0]])
    # Get only the 3 first rows of the colors_mask
