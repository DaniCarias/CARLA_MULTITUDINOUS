...
class CameraManager(object):
    ...
    def listen_sensor(self, image, sensor, world):
        weak_self = weakref.ref(self)
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            print(f"NEW SENSOR: set_sensor: {self.sensors[index][2]}")
            ...
            # FUNCAO DENRO DO LISTEN PARA SALVAR A IMAGEM e parse_image para mostrar na tela
            sensor = self.sensors[index][0]
            world = self._parent.get_world()
            
            self.sensor.listen(lambda image: self.listen_sensor(image, sensor, world))
            
            """ if(self.sensors[index][0].startswith('sensor.lidar')):
                
                world = self._parent.get_world()
                print(f"Lidar! {self.sensors[index][2]}")
                self.sensor.listen(lambda image: image.save_to_disk('_out/lidar_teste/%06d.ply' % image.frame))
                world.tick(seconds=3) """
